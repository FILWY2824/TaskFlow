...
- **任务时间语义修复:统一为开始时间**
  - 服务端列表筛选、过期判断、统计计数改为基于 `start_at`,旧 `due_at` 仅作为历史字段兜底。
  - Web / Windows 共用前端的新建、编辑、日历、日期详情、专注候选任务、统计与本地提醒文案统一为“开始时间”。
  - Android Room 缓存新增 `start_at`,工作台、日历、编辑页和统计页统一展示开始时间。
- **完成态归属修复**
  - 过期或未过期任务点击完成后,不会从当前视图静默消失,而是进入该视图的“已完成”内容。
  - 今日 / 本周 / 近 7 天 / 近 30 天视图会保留在当前区间完成的历史过期任务。
- **Android:工作台筛选与搜索重新收口**
  - 工作台筛选改为两个紧凑下拉选择器,不再把所有筛选项铺满页面。
  - 搜索框改为顶部栏展开,不再占用任务列表内容区。
  - 空态只显示简洁提示,不再展示新建任务引导。
  - 优先级展示对齐 Web 的“无 / 低 / 中 / 高 / 紧急”,不再显示 P0 / P4。
...
- **版本:更新到 V1.4.1**
  ...
  - 实机复查后收口 Android 专注 / 统计卡片视觉,降低厚重阴影并补充概览信息密度。
