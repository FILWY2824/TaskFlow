...
class AlarmForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        // 用户偏好的自动停响时长。AlarmReceiver 注入,缺省退回 90s 与历史一致。
        val autoSnoozeMs = intent?.getLongExtra(AlarmScheduler.EXTRA_AUTO_SNOOZE_MS, 90_000L)
        ...
        // 安全超时:用户偏好里的"响铃自动停止时长"
        safetyHandler = Handler(Looper.getMainLooper())
        safetyRunnable = Runnable {
            Log.i(TAG, "auto-stop after ${autoSnoozeMs}ms")
            stopAlarmAndService()
        }
        safetyHandler?.postDelayed(safetyRunnable!!, autoSnoozeMs)
    ...
    companion object {
        ...
        const val ACTION_STOP = "com.example.taskflow.ACTION_STOP_ALARM"
