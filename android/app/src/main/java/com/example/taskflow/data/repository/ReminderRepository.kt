...
import com.example.taskflow.data.local.LocalAlarmLogEntity
...
class ReminderRepository(
    ...
    private fun useSystemClockBackup(): Boolean = prefs?.current()?.useSystemAlarmClock == true
    ...
    suspend fun refreshAll(): Result<List<ReminderDto>> {
        ...
        if (r is Result.Success) {
            ...
            tokenManager.current().userId?.let { uid -> scheduler.rescheduleAll(uid, useSystemClockBackup()) }
        }
    ...
    suspend fun create(input: ReminderInput): Result<ReminderDto> {
        ...
        if (r is Result.Success) {
            ...
            scheduler.schedule(toEntity(r.data), useSystemClockBackup())
        }
        return r
    }
