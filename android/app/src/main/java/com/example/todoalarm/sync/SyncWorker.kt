...
import com.example.todoalarm.data.remote.SyncEvent
import com.example.todoalarm.data.repository.ReminderRepository
import com.example.todoalarm.data.repository.safeCall
...
class SyncWorker(appContext: Context, params: WorkerParameters)
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            if (me is com.example.todoalarm.data.repository.Result.Error) {
            ...
            while (true) {
                ...
                if (r is com.example.todoalarm.data.repository.Result.Error) {
                    Log.w(TAG, "syncPull failed: ${r.message}")
                    return Result.retry()
                }
                val page = (r as com.example.todoalarm.data.repository.Result.Success).data
        ...
        when (ev.entity_type) {
            "reminder" -> when (ev.action) {
                "deleted" -> {
                    ...
                    if (r is com.example.todoalarm.data.repository.Result.Success) {
    ...
    companion object {
        ...
        private const val UNIQUE_NAME = "todoalarm_sync"
