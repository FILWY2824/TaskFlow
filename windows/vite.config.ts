...
export default defineConfig(async ({ mode }) => {
  // Vite normally loads .env from config.root (../web here). The Windows client
  // intentionally has no windows/.env or web/.env; load the project root .env
  // once and expose only the client-safe VITE_* key to import.meta.env.
  const rootEnv = loadEnv(mode, projectRoot, ['VITE_', 'TASKFLOW_'])
  const defaultServer = (rootEnv.VITE_TASKFLOW_DEFAULT_SERVER || rootEnv.TASKFLOW_DEFAULT_SERVER_URL || '').trim()
  if (defaultServer) {
